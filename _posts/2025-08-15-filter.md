...
1. Normalize
   1. Convert homoglyphs into their likely ASCII target
   2. Convert l33tsp33k into their likely ASCII target
   3. Convert message into NFKD
2.  Apply multiple detection strategies in parallel. If any fail, reject the message
    1.  Direct: Check if blocked words appear directly in the normalized text
    2.  Reverse: Detect backwards spelling attempts (e.g., "nub" → "bun")
    3.  Nospaces: Find matches when spaces are removed from the text
    4.  Unique: Remove duplicate letters and check (e.g., "bunnnnn" → "bun")
    5.  Levenshtein: Calculate edit distance with spell-checking to avoid false positives
